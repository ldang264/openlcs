 ...
 */
public class Q00733s {
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        int oldColor = image[sr][sc];
        if (oldColor != newColor) { // 相等就没必要染了
    ...
    private void dfs(int[][] image, int sr, int sc, int newColor, int oldColor) {
        if (sr >= 0 && sr < image.length && sc >= 0 && sc < image[0].length && image[sr][sc] == oldColor) {
            ...
            dfs(image, sr + 1, sc, newColor, oldColor); // 下一行
            dfs(image, sr - 1, sc, newColor, oldColor); // 上一行
            dfs(image, sr, sc + 1, newColor, oldColor); // 右一列
            dfs(image, sr, sc - 1, newColor, oldColor); // 左一列
        }
    }
}
