...
/**
 * 使用FIFO队列提供一个用来实现阻塞锁和同步操作的框架
 * 该类使用一个单一原子的状态(state)用来实现大部分同步器
 * 子类必须实现protected方法来控制state，获取或释放对象与state的使用相关
 * 子类可以定义其他状态，但只能使用state相关的三个方法进行同步操作
 *
 * 子类应该定义非public内部方法操作同步的属性
 * 本类不实现任何同步接口，而是定义一些模板方法去关联复杂的锁和相关的同步操作
 *
 * 本类同时支持排他和共享模式
 * 当获取排他锁时，其他线程无法再次获取锁；共享锁可以进入共享锁占有的线程
 * 本类不带有逻辑含义，这需要子类同步器自己规定
 ...
 * <h3>用法</h3>
 *
 * 实现以下方法，在其中使用{@link #getState}, {@link
 *  * #setState} and/or {@link #compareAndSetState}操作state
 *
 * <ul>
 * <li> {@link #tryAcquire}
 * <li> {@link #tryRelease}
...
public abstract class MyAbstractQueuedSynchronizer
    ...
    static final class Node {
        ...
        /** waitStatus 线程在一个条件上等待中 */
        static final int CONDITION = -2;
        /**
         * waitStatus 表示下一个acquireShared需要无条件的传播
         */
